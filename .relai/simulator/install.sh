...
resolve_relai_sdk_spec() {
  ...
  local sdk_version
  sdk_version="$(resolve_relai_sdk_version)"
  if [ -n "$sdk_version" ]; then
    echo "relai==$sdk_version"
    return
  fi
  echo "RELAI SDK compatibility metadata must include sdk_version." >&2
  exit 1
}
...
resolve_relai_sdk_metadata() {
  ...
  curl -fsSL \
    ...
    "${RELAI_API_URL%/}/api/v1/agent-studio/sdk/compatibility/python/?cli_version=${RELAI_CLI_VERSION}"
}
...
resolve_relai_sdk_version() {
  ...
  version="$(printf '%s' "$metadata" | jq -r '.sdk_version // empty')"
  printf '%s\n' "$version"
}
